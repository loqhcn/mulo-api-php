...
namespace mulo\library\auth\guard;

use mulo\library\auth\lib\Jwt;
...
class JwtGuard
{
    ...
    public function user(bool $failException = false)
    {
        ...
        if (is_null($this->user) && $failException) {
            throw new MuloException('请登录后再继续操作');
        } else if ($this->user) {
            // 检测用户是否正常
    ...
    public function attempt(array $credentials = [], $remember = false)
    {
        if (empty($credentials['account']) || empty($credentials['password'])) {
            throw new MuloException('请输入正确的账号或密码');
        }
        // 查询用户信息
        ...
        if (!$user) {
            throw new MuloException('您的账号或密码不正确');  // 实际没有检测密码
        }
     ...
     * 实例化 provider
     *
     * @return \mulo\library\auth\provider\Admin|\mulo\library\auth\provider\User
     */
    public function getProvider()
    {
        if (!$this->providerClass) {
            $class = "\\mulo\\library\\auth\\provider\\" . ucfirst($this->provider);
